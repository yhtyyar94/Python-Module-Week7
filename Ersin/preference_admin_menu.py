...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        # Doğru dosya yolunu oluşturmak için os.path kullanımı
        image_path = os.path.join(os.path.dirname(__file__), 'images', 'zemin-kucuk.jpg')
        if os.path.exists(image_path):
            # Stil dosyasına dinamik bir dosya yolu eklemek için background-image özelliği
            self.frame.setStyleSheet(f"QFrame{{background-image: url({image_path}); width:800px; height:600px;}}")
        else:
