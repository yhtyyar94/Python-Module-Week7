...
class VITForm(QWidget):
    ...
    def setupUi(self):
        ...
        self.part_time_btn = QRadioButton("Kısmen Uitkering Alıyorum - Parttime Çalışıyorum")
        ...
        self.course_label = create_label("Şu anda bir dil kursuna devam ediyor musunuz?")
        ...
        self.it_course_label = create_label("Başka bir IT kursu aldınız mı? (Detayları yazın)")
        ...
        self.it_experience_label = create_label("Daha önce herhangi bir IT iş tecrübeniz var mı?")
        ...
        self.project_label = create_label("Öğretmenlik projesi veya Leerwerktraject projesine dahil misiniz?")
        ...
        self.it_sector_label = create_label("IT sektöründe hangi bölümde çalışmak istersiniz?")
        ...
        self.motivation_label = create_label("IT sektöründe kariyer yapmak için sizi harekete geçiren motivasyon nedir?")
    ...
    def confirm_submission(self):
        form_data = [
            ...
            self.economic_status_group.checkedButton().text() if self.economic_status_group.checkedButton() else "Boş",
            "Evet" if self.course_yes.isChecked() else ("Hayır" if self.course_no.isChecked() else "Boş"),
            ", ".join([checkbox.text() for checkbox in [self.a1_checkbox, self.a2_checkbox, self.b1_checkbox, self.b2_checkbox] if checkbox.isChecked()]) or "Boş",
            ...
            ", ".join([checkbox.text() for checkbox in [
                self.cloud_checkbox, self.tester_checkbox, self.scrum_checkbox, self.devops_checkbox,
                self.web_checkbox, self.software_checkbox, self.data_checkbox, self.cyber_checkbox
            ] if checkbox.isChecked()]) or "Boş",
    ...
    def submit_form(self, form_data):
        try:
            ...
            if uploaded_file:
                QMessageBox.information(self, "Başarılı", "Veriler başarıyla kaydedildi ve yüklendi.")
            else:
                QMessageBox.critical(self, "Hata", "Excel dosyasını yükleme sırasında bir hata oluştu.")
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Excel dosyasına kaydetme sırasında bir hata oluştu: {e}")
        ...
        try:
            ...
            QMessageBox.critical(self, "Hata", f"E-posta gönderimi sırasında bir hata oluştu: {e}")
    ...
    def show_success_message(self):
        ...
        success_message = QLabel("TEBRİKLER, BAŞARIYLA KAYIT OLDUNUZ.\nVIT PROJESİNE GÖSTERDİĞİNİZ İLGİYE TEŞEKKÜR EDERİZ.")
        success_message.setAlignment(Qt.AlignmentFlag.AlignCenter)
        success_message.setStyleSheet("font-size: 18px; font-weight: bold; text-align: center;")
